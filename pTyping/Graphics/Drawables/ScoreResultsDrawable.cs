...
public class ScoreResultsDrawable : CompositeDrawable {
	...
	public ScoreResultsDrawable(Score score) {
		this.Drawables.Add(
			SetRotOrigin(this._username = new TextDrawable(Vector2.Zero, pTypingGame.JapaneseFontStroked, $"Username: {score.User.Username}", 30))
		);
		this.Drawables.Add(SetRotOrigin(this._score = new TextDrawable(Vector2.Zero, pTypingGame.JapaneseFontStroked, $"Score: {score.AchievedScore}", 30)));
		this.Drawables.Add(
			SetRotOrigin(this._accuracy = new TextDrawable(Vector2.Zero, pTypingGame.JapaneseFontStroked, $"Accuracy: {score.Accuracy * 100:00.00}%", 30))
		);
