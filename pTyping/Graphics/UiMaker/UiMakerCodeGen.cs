	...
	private static void PrintConstructor(IndentedStringBuilder builder, UiMakerElementContainer container) {
		...
		foreach (UiMakerElement element in container.Elements) {
			string args = element.Type switch {
				UiMakerElementType.Text    => $"{FormatVector2(element.Position)}, pTyping.pTypingGame.JapaneseFont, \"{element.Text}\", {element.FontSize}",
				...
				UiMakerElementType.Button =>
					$"{FormatVector2(element.Position)}, pTypingGame.JapaneseFont, {element.FontSize}, \"{element.Text}\", {FormatColor(element.ButtonColor)}, {FormatColor(element.Color)}, {FormatColor(element.ButtonOutlineColor)}, {FormatVector2(element.ButtonSize)}",
